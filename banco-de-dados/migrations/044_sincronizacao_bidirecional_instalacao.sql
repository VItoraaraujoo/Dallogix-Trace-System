-- Liga a instalação local à empresa central também para comandos e estado
-- operacional. Os identificadores remotos permitem que os bancos tenham IDs
-- internos diferentes sem perder a correspondência entre registros.

SET NAMES utf8mb4;

DELIMITER //
CREATE PROCEDURE trace_ensure_bidirectional_sync_columns()
BEGIN
  IF NOT EXISTS (
...
END//
DELIMITER ;
CALL trace_ensure_bidirectional_sync_columns();
DROP PROCEDURE trace_ensure_bidirectional_sync_columns;

DELIMITER //
CREATE PROCEDURE trace_ensure_bidirectional_sync_indexes()
BEGIN
  IF NOT EXISTS (
...
END//
DELIMITER ;
CALL trace_ensure_bidirectional_sync_indexes();
DROP PROCEDURE trace_ensure_bidirectional_sync_indexes;
