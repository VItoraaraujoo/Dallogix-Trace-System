...
miss="$(curl -sS -b "$cookie_file" "$base_url/api/romaneios.php?number=${number}&status=FINALIZADO")"
if printf '%s' "$miss" | grep -q "$number"; then
...
echo "OK: sessão, produtos, criação/listagem de romaneio, duplicidade e filtros validados."
