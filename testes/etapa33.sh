...
roles="$(docker compose -f "$root/docker-compose.yml" exec -T mysql mysql -N -utrace -pchange-me-local trace_local -e "SHOW COLUMNS FROM users LIKE 'role'" 2>/dev/null)"
