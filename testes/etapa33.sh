...
roles="$(docker compose -f "$root/docker-compose.yml" exec -T mysql mysql -N -utrace -p"${MYSQL_PASSWORD:-}" "${MYSQL_DATABASE:-trace_local}" -e "SHOW COLUMNS FROM usuarios LIKE 'role'" 2>/dev/null)"
