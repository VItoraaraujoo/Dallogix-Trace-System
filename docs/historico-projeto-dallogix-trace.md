...
│   │   └── screens/         # telas separadas
│   └── index.html
├── integration/
│   └── industrial/          # integração CLP/gateway/mapa de registradores
├── tests/                   # testes automatizados por etapa
├── docs/
│   ├── arquitetura/
│   ├── pendencias/
│   └── seguranca/
...
O protótipo funcional local está estruturado e executável via Docker, com frontend modular, API PHP, banco MySQL, dashboard empresarial, configurações de máquinas e base para integração industrial.
