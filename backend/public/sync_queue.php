...
if (!$queueId) json_response(['error' => 'Id do evento é obrigatório.'], 422);
...
if (!$event) json_response(['error' => 'Evento não encontrado ou não disponível para retry.'], 404);

$remoteUrl = trim((string) (getenv('SYNC_REMOTE_URL') ?: ''));
if ($remoteUrl === '') {
    json_response(['data' => ['id' => (int) $event['id'], 'status' => $event['status'], 'processed' => false, 'reason' => 'SYNC_REMOTE_URL não configurada']], 202);
}

$updateProcessing = $pdo->prepare("UPDATE sync_queue SET status = 'PROCESSANDO', attempts = attempts + 1 WHERE id = :id");
$updateProcessing->execute(['id' => $queueId]);
$requestBody = json_encode(['event_uuid' => $event['event_uuid'], 'aggregate_type' => $event['aggregate_type'], 'aggregate_id' => (int) $event['aggregate_id'], 'payload' => json_decode($event['payload'], true)], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
$context = stream_context_create(['http' => ['method' => 'POST', 'header' => "Content-Type: application/json\r\n", 'content' => $requestBody, 'timeout' => 5, 'ignore_errors' => true]]);
$response = @file_get_contents($remoteUrl, false, $context);
$responseHeaders = get_defined_vars()['http_response_header'] ?? [];
$success = $response !== false && isset($responseHeaders[0]) && preg_match('/\s2\d\d\s/', $responseHeaders[0]) === 1;

if ($success) {
    $done = $pdo->prepare("UPDATE sync_queue SET status = 'ENVIADO', last_error = NULL WHERE id = :id");
    $done->execute(['id' => $queueId]);
    json_response(['data' => ['id' => (int) $queueId, 'status' => 'ENVIADO', 'processed' => true]]);
}

$error = 'Falha ao enviar para o endpoint remoto.';
$failed = $pdo->prepare("UPDATE sync_queue SET status = 'ERRO', last_error = :last_error, available_at = DATE_ADD(NOW(), INTERVAL LEAST(attempts * 5, 300) MINUTE) WHERE id = :id");
$failed->execute(['id' => $queueId, 'last_error' => $error]);
json_response(['error' => $error, 'data' => ['id' => (int) $queueId, 'status' => 'ERRO']], 502);
