...
final class ServicoMonitoramento
{
    ...
    public function snapshot(int $companyId, int $periodDays = 30, int $clpSignalLimit = 3): array
    {
        ...
        $maquinas = $query("SELECT e.id, e.equipment_code, e.name, CASE WHEN d.status = 'ONLINE' AND d.last_seen_at >= DATE_SUB(NOW(3), INTERVAL {$clpSignalLimit} SECOND) THEN 'ONLINE' WHEN d.status = 'ERRO' THEN 'ERRO' ELSE 'OFFLINE' END AS clp_status, d.last_seen_at, d.details AS clp_details, c.id AS carregamento_id, c.state AS carregamento_state, r.id AS romaneio_id, r.number AS romaneio_number, rt.plate, COALESCE((SELECT SUM(ri.planned_quantity) FROM romaneio_itens ri WHERE ri.romaneio_id = c.romaneio_id AND (ri.truck_id = c.truck_id OR ri.truck_id IS NULL)), 0) AS planned_quantity, COALESCE(c.leituras_validas, 0) AS valid_readings FROM equipamentos e LEFT JOIN status_dispositivos d ON d.equipment_id = e.id AND d.device_type = 'CLP' LEFT JOIN carregamentos c ON c.id = (SELECT c2.id FROM carregamentos c2 WHERE c2.equipment_id = e.id AND c2.state <> 'FINALIZADO' AND EXISTS (SELECT 1 FROM romaneios r2 WHERE r2.id = c2.romaneio_id AND r2.status NOT IN ('FINALIZADO', 'CANCELADO')) ORDER BY c2.id DESC LIMIT 1) LEFT JOIN romaneios r ON r.id = c.romaneio_id LEFT JOIN romaneio_caminhoes rt ON rt.id = c.truck_id WHERE e.company_id = :company_id ORDER BY e.equipment_code", ["company_id" => $companyId]);
