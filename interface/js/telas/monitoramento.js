...
export function tablet(store) {
  const left = Math.max(0, store.state.planned - store.state.loaded);
  const last = store.state.monitoring?.ultima_leitura;
  const emergency = store.state.emergency;
  const canUnlock =
    ["ADMIN_EMPRESA", "SUPERVISOR"].includes(store.state.userRole) &&
    Boolean(store.state.loadingId);
  return `${pageHeader("Monitoramento", "Monitor tablet", "Tela web simplificada para acompanhar o carregamento no caminhão.")}
  <div class="tablet ${emergency ? "tablet-emergency" : ""}"><div class="tablet-topline"><span class="kicker">Romaneio #${esc(store.state.romaneio)} • caminhão ${esc(store.state.truck)}</span><span class="tablet-live-dot">● Atualização automática</span></div><h2>${emergency ? "Carregamento bloqueado" : "Carregamento atual"}</h2>${emergency ? `<div class="tablet-blocked"><strong>EMERGÊNCIA ATIVA</strong><span>A contagem está bloqueada pelo CLP.</span>${canUnlock ? button("Desbloquear máquina", "unlock", "secondary") : ""}</div>` : ""}<div class="grid three"><div class="metric"><small>Programado</small><strong>${store.state.planned}</strong></div><div class="metric"><small>Carregado</small><strong>${store.state.loaded}</strong></div><div class="metric"><small>Faltam</small><strong>${left}</strong></div></div>${progress(store)}<div class="tablet-last-read"><small>Última leitura</small><strong>${last?.barcode ? esc(last.barcode) : "Aguardando leitura"}</strong><span>${last?.result ? esc(last.result) : "Sensor pronto"}</span></div>${!emergency && left <= 5 && left > 0 ? '<div class="alert-box">⚠ Faltam 5 sacas para o fim do carregamento</div>' : !emergency ? "<p>Operação em acompanhamento local.</p>" : ""}</div>`;
}
