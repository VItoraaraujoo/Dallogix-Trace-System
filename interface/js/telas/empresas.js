...
import { pageHeader, companyGrid } from "../funcoes/view.js?v=202609220100";
...
export function companies(store) {
  ...
  const activationPanel = activation ? `<section class="panel company-activation-panel" role="status"><div><span class="kicker">${activation.activation_code ? "Código de ativação" : "Empresa criada"}</span><h3>${activation.activation_code ? "Código de ativação da empresa" : "Próximo passo"}</h3>${activation.activation_code ? `<p>Este código é único, permanente e será informado uma vez no PC industrial.</p><strong class="company-activation-code">${esc(activation.activation_code)}</strong>` : "<p>Ative a licença da empresa para liberar o código de ativação do PC industrial.</p>"}<small>Empresa: ${esc(activation.name)} · Login: @${esc(activation.login_domain)}</small></div>${activation.activation_code ? `<button class="button secondary" data-action="copy-company-activation" data-code="${esc(activation.activation_code)}" type="button">Copiar código</button>` : ""}</section><br>` : "";
